...
/**
 @author MEEPOfFaith
*/
open class ContinuousWeapon : Weapon{
    ...
    override fun update(unit: mindustry.gen.Unit, mount: WeaponMount){
        ...
        if(rotate && (mount.rotate || mount.shoot) && can){
            var axisX = unit.x + Angles.trnsx(unit.rotation - 90,  x, y)
            var axisY = unit.y + Angles.trnsy(unit.rotation - 90,  x, y)
        ...
        var weaponRotation = unit.rotation - 90 + (rotate ? mount.rotation : baseRotation)
        var mountX = unit.x + Angles.trnsx(unit.rotation - 90, x, y)
        var mountY = unit.y + Angles.trnsy(unit.rotation - 90, x, y)
        var bulletX = mountX + Angles.trnsx(weaponRotation, this.shootX, this.shootY)
        var bulletY = mountY + Angles.trnsy(weaponRotation, this.shootX, this.shootY)
        var shootAngle = bulletRotation(unit, mount, bulletX, bulletY)
        ...
        if(!controllable && autoTarget){
            ...
            if(mount.target !is null && checkTarget(unit, mount.target, mountX, mountY, bullet.range)){
                mount.target = null
            }
            
            var shoot = false
            
            if(mount.target !is null){
                shoot = mount.target.within(mountX, mountY, bullet.range + Math.abs(shootY) + (mount.target instanceof Sized s ? s.hitSize()/2f : 0f)) && can
                
                if(predictTarget){
                    var to = Predict.intercept(unit, mount.target, bullet.speed)
                    mount.aimX = to.x
                    mount.aimY = to.y
                }else{
        ...
        var shoot = mount.shoot && //must be shooting
        ...
        if(mount.bullet !is null){
            if(!mount.bullet.isAdded() || mount.bullet.type !is bullet){
                mount.bullet = null
            }else{
                ...
                if(shootSound !is Sounds.none && !headless){
